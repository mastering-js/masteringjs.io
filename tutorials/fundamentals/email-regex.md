To validate emails using a regular expression, you can use the `match` function.
You should not use the `pattern` option on the `input` tag because it uses unicode, and forward slashes are affected by this.
The regular expression used for this example is not unicode compatible.
